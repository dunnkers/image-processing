...
The first step is to calculate the 2D Discrete Fourier Transform (DFT) of the image, according to Formula \ref{fourier2d}. Since we are allowed to use Matlab's \textit{fft2} function we did not implement this ourselves.

\begin{equation}\label{fourier2d}
    F(u, v)=\sum_{x=0}^{M-1} \sum_{y=0}^{N-1} f(x, y) e^{-j 2 \pi(u x / M+v y / N)}
\end{equation}

For the second step we need to center the Fourier transform. Figure \ref


\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{Assignment_2/output_plots/}
    \caption{}
    \label{fig:fouriershiftFromBook}
\end{figure}

matlab abs is book formula



\begin{equation}\label{fouriershift}
